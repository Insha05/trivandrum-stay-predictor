...
st.markdown("Developed by: *[Your Name]*, 2nd Year AI & DS")

# 1. Sidebar Inputs
st.sidebar.header("Stay Details")
stay_type = st.sidebar.radio("Select Stay Type", ["Full Apartment", "PG / Hostel"])
...
if stay_type == "Full Apartment":
    sqft = st.sidebar.number_input("Enter Square Feet", min_value=300, max_value=3000, value=1000)
    base_price = sqft * 15 # ₹15 per sqft
    
    # Location Multipliers
    multipliers = {"Technopark Phase 1": 1.2, "Technopark Phase 3": 1.1, "Kazhakkoottam": 1.0}
    final_price = base_price * multipliers[loc]
    label = "Estimated Monthly Rent"

else: # PG / Hostel Logic
    room_type = st.sidebar.selectbox("Room Type", ["Single Room", "2-Sharing", "3-Sharing"])
    pg_rates = {"Single Room": 12000, "2-Sharing": 7500, "3-Sharing": 5500}
    
    # PGs are slightly more expensive near Phase 1
    loc_extra = {"Technopark Phase 1": 1000, "Technopark Phase 3": 500, "Kazhakkoottam": 0}
...
if st.button("Calculate Market Value"):
    ...
    with st.expander("See AI Logic"):
        ...
        st.write("For PGs: Classification-based pricing (Fixed rate + Location Premium).")
